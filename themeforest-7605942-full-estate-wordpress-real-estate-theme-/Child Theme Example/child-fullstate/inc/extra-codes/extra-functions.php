...
if(function_exists('optionsframework_init')){
    ...
    if (! function_exists('maison_colors') ){
        function maison_colors($maison_component){
        $maison_arr_color = '';
        if(of_get_option('general_skin_color_ch')){
            switch ($maison_component){
                case 'a':
                    if(of_get_option('general_color_a_ch')){$maison_arr_color = 'style="color: '.of_get_option('general_color_a').';"';}
                    break;
                case 'p':
                    if(of_get_option('general_color_p_ch')){$maison_arr_color = 'style="color: '.of_get_option('general_color_p').';"';}
                    break;
                case 'h1':
                    if(of_get_option('general_color_h1_ch')){$maison_arr_color = 'style="color: '.of_get_option('general_color_h1').';"';}
                    break;
                case 'h2':
                    if(of_get_option('general_color_h2_ch')){$maison_arr_color = 'style="color: '.of_get_option('general_color_h2').';"';}
                    break;
                case 'h3':
                    if(of_get_option('general_color_h3_ch')){$maison_arr_color = 'style="color: '.of_get_option('general_color_h3').';"';}
                    break;
                case 'h4':
                    if(of_get_option('general_color_h4_ch')){$maison_arr_color = 'style="color: '.of_get_option('general_color_h4').';"';}
                    break;
                case 'h5':
                    if(of_get_option('general_color_h5_ch')){$maison_arr_color = 'style="color: '.of_get_option('general_color_h5').';"';}
                    break;
                case 'h6':
                    if(of_get_option('general_color_h6_ch')){$maison_arr_color = 'style="color: '.of_get_option('general_color_h6').';"';}
                    break;
                case 'h1s':
                    if(of_get_option('general_color_h1s_ch')){$maison_arr_color = 'style="color: '.of_get_option('general_color_h1s').';"';}
                    break;
                case 'btn':
                    if(of_get_option('general_color_btn_ch')){$maison_arr_color = 'style="background-color: '.of_get_option('general_color_btn').';"';}
                    break;
                case 'menu':
                    if(of_get_option('general_color_menu_ch')){$maison_arr_color = 'style="background-color: '.of_get_option('general_color_menu').';"';}
                    break;
                //EXTRAS
                
                
                case 'top':
                    if(of_get_option('general_color_top_ch')){$maison_arr_color = 'style="background-color: '.of_get_option('general_color_top').';"';}
                    break;
                case 'std':
                    if(of_get_option('general_color_std_ch')){$maison_arr_color = 'style="background-color: '.of_get_option('general_color_std').';"';}
                    break;           
                case 'soc':
                    if(of_get_option('general_color_social_ch')){$maison_arr_color = 'style="background-color: '.of_get_option('general_color_social').';"';}
                    break;
                case 'footer':
                    if(of_get_option('general_color_footer_ch')){$maison_arr_color = 'style="background-color: '.of_get_option('general_color_footer').';"';}
                    break;
                case 'sfooter':
                    if(of_get_option('general_color_sf_ch')){$maison_arr_color = 'style="background-color: '.of_get_option('general_color_sf').';"';}
                    break;
                case 'search':
                    if(of_get_option('general_color_search_ch')){
                        $maison_arr_color = 'style="background-color: '.of_get_option('general_color_search1');
                        $maison_arr_color .= ' border-button: 3px solid '.of_get_option('general_color_search1').';"';
                    }
                    break;
                case 'search1':
                    if(of_get_option('general_color_search_ch')){
                        $maison_arr_color = 'style="background-color: '.of_get_option('general_color_search2').';"';
                    }
                    break;
                case 'plan':
                    if(of_get_option('general_color_plan_ch')){
                        $maison_arr_color = 'style="background-color: '.of_get_option('general_color_plan1').';"';
                    }
                    break;            
                case 'plan1':
                    if(of_get_option('general_color_plan_ch')){
                        $maison_arr_color = 'style="background-color: '.of_get_option('general_color_plan2').';"';
                    }
                    break;            
            }
        
        }
        return $maison_arr_color;
    }
}

if( !function_exists( 'maison_config_styles' ) ) {
        function maison_config_styles(){
            $maison_styles = '';
            $maison_style = '';
            
        //TYPOGRAPHY CONFIGURATION
        if(!is_admin()){
            for($maison_i=1; $maison_i<=6; $maison_i++){
                if(of_get_option('general_color_h'.$maison_i.'_ch') == '1'){
                    $maison_styles = of_get_option('general_color_h'.$maison_i);
                    $maison_style .= 'h'.$maison_i.'{color: '.$maison_styles['color'].' !important; font-style: '.$maison_styles['style'].' !important; font-family: '.$maison_styles['face'].'!important; font-size: '.$maison_styles['size'].' !important;}';
                }
            }  
            if(of_get_option('general_color_h1s_ch') == '1'){
                $maison_styles = of_get_option('general_color_h1s');
                $maison_style .= 'small{color: '.$maison_styles['color'].' !important; font-style: '.$maison_styles['style'].' !important; font-family: '.$maison_styles['face'].'!important; font-size: '.$maison_styles['size'].' !important;}';
            }
            if(of_get_option('general_color_a_ch') == '1'){
                $maison_styles = of_get_option('general_color_a');
                $maison_style .= 'a{color: '.$maison_styles['color'].' !important; font-style: '.$maison_styles['style'].' !important; font-family: '.$maison_styles['face'].'!important; font-size: '.$maison_styles['size'].' !important;}';
            }
            if(of_get_option('general_color_p_ch') == '1'){
                $maison_styles = of_get_option('general_color_p');
                $maison_style .= 'p{color: '.$maison_styles['color'].' !important; font-style: '.$maison_styles['style'].' !important; font-family: '.$maison_styles['face'].'!important; font-size: '.$maison_styles['size'].' !important;}';
            }
            
            if(of_get_option('general_skin_color_ch')){
                echo '<style>'.$maison_style.'</style>';
            }
...
if (! function_exists('maison_pagination')){
    function maison_pagination($pages = '', $range = 3){ 
        $showitems = ($range * 3)+1;
...
if(!function_exists('maison_time_ago')){
    function maison_time_ago($maison_year='2000',$maison_month='01',$maison_day='01'){
        
        $maison_fecha_pos = new DateTime($maison_year.'-'.$maison_month.'-'.$maison_day);
        $maison_fecha_act = new DateTime(date('Y-m-d'));        

        $maison_interval = $maison_fecha_pos->diff($maison_fecha_act);

        $maison_year   = $maison_interval->format('%y');
        $maison_month  = $maison_interval->format('%m');
        $maison_day    = $maison_interval->format('%d');

        if($maison_year > 0){
            return $maison_year.' years ago';            
        }else if($maison_month > 0){
            return $maison_month.' months ago';
        }else if($maison_day > 1){
            return ($maison_day-1).' days ago';
        }
        return 'Right now!!!';
    }
}

if( !function_exists( 'maison_insert_br' ) ) {
    function maison_insert_br($maison_string){
        $maison_cad1 = '';
        $maison_cad2 = '';
        $maison_cad1 = $maison_string;
        $maison_order   = array("\r\n", "\n", "\r");
        $maison_replace = '<br />';
        $maison_cad2 = str_replace($maison_order, $maison_replace, $maison_cad1);
